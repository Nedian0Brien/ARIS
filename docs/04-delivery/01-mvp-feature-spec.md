...
- 대화 스트림 + 컴포저
- 개별 세션 진입 경로 `/sessions/[sessionId]`
...
### F9. Session Management (신규)
- 세션 목록 조회 및 신규 세션 생성 UI
- 기본 진입 경로 `/`
- 시작 위치(Path) 및 에이전트(Flavor) 선택 기능

### F10. Hybrid Workspace Access (신규)
- 호스트 시스템과의 프로젝트 연동 지원
- Docker Volume 마운트를 통한 실제 프로젝트 파일 수정 권한 부여
- `HOST_PROJECTS_ROOT` 설정을 통해 에이전트의 작업 범위 제어

수용 기준:
- 신규 세션 생성 후 즉시 해당 워크스페이스로 이동한다.
- 현재 실행 중인 모든 세션을 한눈에 파악할 수 있다.
