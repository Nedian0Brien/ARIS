...
- 도메인(`aris.lawdigest.kr`)은 HTTPS로만 노출
...
- `diagrams/aris-runtime-architecture.html`: 현재 운영 기준의 런타임 아키텍처 다이어그램
- `diagrams/aris-runtime-architecture.architecture.json`: Archify 재렌더링용 JSON IR

다이어그램의 배포 경계와 production URL은 `deploy/README.md`의 최신 기준을 우선한다.
