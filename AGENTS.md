...
- 모바일 UI 또는 긴 텍스트가 포함된 UI를 수정할 때는 완료 전에 기본적으로 `services/aris-web/tests/mobileOverflowLayout.test.ts` 를 실행해 overflow 회귀를 확인한다.
- `services/aris-web/tests/e2e/mobile-overflow.spec.ts` 는 아래 경우에만 필수로 실행한다: 레이아웃 셸/페이지 전환/모달·드로어/viewport·scroll 동작을 건드린 고위험 변경, 여러 화면에 걸친 구조 리팩토링, 사용자가 명시적으로 요구한 경우.
- 위 조건에 해당하지 않는 일반적인 UI 문구·스타일·국소 컴포넌트 수정에서는 `mobileOverflowLayout.test.ts` 만 통과하면 e2e는 생략할 수 있다. e2e를 생략한 경우에는 최종 보고에 생략 사유를 반드시 남긴다.
