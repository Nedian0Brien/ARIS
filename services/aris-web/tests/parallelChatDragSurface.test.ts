...
describe('project parallel chat drag surface', () => {
  it('renders split panels in-process instead of embedding the app in iframes', () => {
    ...
    expect(homeClient).toContain('buildProjectRuntimeEventsPath(projectId, params)');
  ...
  it('makes project sidebar chat children draggable', () => {
    ...
    expect(homeClient).toContain('onProjectChatDragStart(event, projectId, chat)');
    expect(homeClient).toContain('onProjectChatDragEnd={handleProjectChatDragEnd}');
    expect(homeClient).toContain('writeProjectChatDragPayload(event, projectId, chat)');
    expect(homeClient).toContain('projectId: parsedProjectId');
    expect(homeClient).not.toContain('sessionId: parsed.sessionId');
  ...
  it('persists project panel layout per project workspace', () => {
    expect(homeClient).toContain('createProjectPanelLayoutStorageKey(projectId)');
