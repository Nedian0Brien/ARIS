...
const mocks = vi.hoisted(() => ({
  ...
  getProjectRealtimeEvents: vi.fn(),
...
vi.mock('@/lib/happy/client', () => ({
  streamProjectEvents: mocks.streamProjectEvents,
  getProjectRealtimeEvents: mocks.getProjectRealtimeEvents,
  HappyHttpError: mocks.HappyHttpError,
}));

import { GET } from '@/app/api/runtime/projects/[projectId]/events/stream/route';
...
function buildEvent(overrides: Partial<UiEvent> = {}): UiEvent {
  return {
    ...
    body: overrides.body ?? 'path: services/aris-web/app/ProjectDashboard.tsx',
    meta: {
      chatId: 'chat-1',
...
describe('session events stream route', () => {
  ...
  it('replays the initial event page when the stream starts without an after cursor', async () => {
    ...
    mocks.getProjectRealtimeEvents.mockResolvedValue({ events: [], cursor: 0 });

    // DB events: 첫 번째 호출에서 이벤트 반환, 이후 404로 스트림 종료
    mocks.streamProjectEvents
    ...
    const response = await GET(
      new NextRequest('http://localhost/api/runtime/projects/session-1/events/stream?chatId=chat-1'),
      { params: Promise.resolve({ projectId: 'session-1' }) },
    );

    const payload = await response.text();

    expect(mocks.streamProjectEvents).toHaveBeenCalledWith('session-1', {
      after: undefined,
      limit: 40,
      chatId: 'chat-1',
    ...
    expect(mocks.getProjectRealtimeEvents).toHaveBeenCalledWith({
      projectId: 'session-1',
      afterCursor: 0,
      chatId: 'chat-1',
    });
  ...
  it('delivers realtime events before DB events in each polling cycle', async () => {
    ...
    mocks.getProjectRealtimeEvents
    ...
    const response = await GET(
      new NextRequest('http://localhost/api/runtime/projects/session-1/events/stream'),
      { params: Promise.resolve({ projectId: 'session-1' }) },
    );
  ...
  it('does not enqueue late events after the request aborts', async () => {
    ...
    mocks.getProjectRealtimeEvents
      ...
      .mockResolvedValue({ events: [], cursor: 1 });
    mocks.streamProjectEvents.mockResolvedValue({ events: [], latestSeq: 0 });
