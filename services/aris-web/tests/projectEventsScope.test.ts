...
import { getScopedProjectEvents } from '@/lib/hooks/useProjectEvents';
...
describe('getScopedProjectEvents', () => {
  it('returns an empty list when the current events belong to a different chat', () => {
    ...
    expect(getScopedProjectEvents(staleEvents, 'chat-old', 'chat-new')).toEqual([]);
  });
  ...
  it('returns the original events when the active chat matches', () => {
    ...
    expect(getScopedProjectEvents(currentEvents, 'chat-new', 'chat-new')).toEqual(currentEvents);
  });
});
