...
function persistDebugReport(testInfo: TestInfo, report: unknown) {
  const reportPath = testInfo.outputPath('chat-scroll-entry-refresh-debug.json');
  writeFileSync(reportPath, JSON.stringify(report, null, 2));
  console.log(`chat-scroll-entry-refresh-debug report: ${reportPath}`);
}

test('captures mobile scroll traces for existing chat entry and reload', async ({ page, context }, testInfo) => {
  ...
  const entryReport = await captureScrollReport(page, 'existing-chat-entry');

  await resetScrollDebug(page);
  await page.reload({ waitUntil: 'commit', timeout: 45_000 });
  await page.locator('[class*="ChatInterface_chatShell"]').first().waitFor({ state: 'visible', timeout: 30_000 });
  const reloadReport = await captureScrollReport(page, 'page-reload');
  ...
  expect(entryReport.events.length).toBeGreaterThan(0);
  expect(entryReport.events.some((event) => String(event.source).startsWith('tail:restore-entry:'))).toBe(true);
  expect(reloadReport.events.length).toBeGreaterThan(0);
});
