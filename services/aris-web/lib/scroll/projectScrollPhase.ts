export type ProjectScrollPhase =
  | 'idle'
  | 'user-scrolling'
  | 'restoring-tail'
  | 'loading-older'
...
export type ProjectScrollPhaseEvent =
  | 'resume-start'
  | 'scroll-observed'
  | 'viewport-changed'
  | 'resume-stable'
...
type ResolveProjectScrollPhaseInput = {
  currentPhase: ProjectScrollPhase;
  event: ProjectScrollPhaseEvent;
};

export function resolveProjectScrollPhase(input: ResolveProjectScrollPhaseInput): ProjectScrollPhase {
  switch (input.event) {
    case 'resume-start':
      return 'resuming';
