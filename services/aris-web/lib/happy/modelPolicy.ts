...
const DEFAULT_BUILTIN_MODELS_BY_AGENT: Record<SupportedAgent, readonly string[]> = {
  codex: ['gpt-5.4', 'gpt-5.3-codex', 'gpt-5', 'gpt-5-mini'],
  claude: ['claude-sonnet-4-6', 'claude-opus-4-6', 'claude-haiku-4-5'],
  gemini: ['auto-gemini-3', 'gemini-3-flash', 'gemini-2.5-pro', 'gemini-2.5-flash', 'gemini-2.0-flash'],
};
...
function loadModelPolicyConfig(): RuntimeModelPolicyConfig {
  ...
  try {
    ...
    const legacyAliases = (() => {
      ...
      return entries.length > 0 ? Object.fromEntries(entries) : DEFAULT_LEGACY_ALIASES;
    })();
