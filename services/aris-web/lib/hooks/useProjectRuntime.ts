...
export function useProjectRuntime(projectId: string, chatId?: string | null, enabled = true) {
  const cacheKey = `${projectId}:${chatId?.trim() || '__default__'}`;
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const query = chatId && chatId.trim().length > 0
          ...
          : '';
        const response = await fetch(`/api/runtime/projects/${encodeURIComponent(projectId)}/runtime${query}`, {
          cache: 'no-store',
        });
    ...
    if (typeof WebSocket === 'function') {
      realtimeSocket = new WebSocket(buildRuntimeEventChannelUrl({ projectId, chatId }));
  ...
  }, [cacheKey, projectId, chatId, enabled]);
