...
export function parseProjectSyncLeaderRecord(raw: string | null): ProjectSyncLeaderRecord | null {
  if (!raw) {
    return null;
  }

  try {
    const parsed = JSON.parse(raw) as Partial<ProjectSyncLeaderRecord>;
    if (
...
export function shouldClaimProjectSyncLeadership(
  record: ProjectSyncLeaderRecord | null,
  now: number,
  tabId: string,
  isVisible: boolean,
  isFocused: boolean,
...
export function useProjectSyncLeader(projectId: string, scopeKey: string | null = null) {
  const tabIdRef = useRef(`project-sync-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`);
  ...
  useEffect(() => {
    ...
    const storageKey = normalizedScopeKey
      ? `aris:project-sync-leader:${projectId}:${normalizedScopeKey}`
      : `aris:project-sync-leader:${projectId}`;
    let released = false;

    const releaseLeadership = () => {
      const current = parseProjectSyncLeaderRecord(readLocalStorage(storageKey));
    ...
    const writeLeadership = (focused: boolean) => {
      const ok = writeLocalStorage(
        ...
        JSON.stringify({
          ...
          updatedAt: Date.now(),
          focused,
        } satisfies ProjectSyncLeaderRecord),
      );
      if (ok) {
        setIsLeader(true);
        return;
    ...
    const syncLeadership = () => {
      ...
      const now = Date.now();
      const current = parseProjectSyncLeaderRecord(readLocalStorage(storageKey));
      if (shouldClaimProjectSyncLeadership(current, now, tabIdRef.current, visible, focused)) {
