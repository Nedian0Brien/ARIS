...
import type { ProjectEventsPage, UiEvent } from '@/lib/happy/types';
...
export function getScopedProjectEvents(
  events: UiEvent[],
...
export function useProjectEvents(
  ...
  const scopedEvents = useMemo(
    () => getScopedProjectEvents(events, eventsForChatId, chatId),
  ...
  const refreshEvents = useCallback(async (force = false) => {
    ...
    const response = await fetch(
      `/api/runtime/projects/${encodeURIComponent(projectId)}/events${query ? `?${query}` : ''}`,
      { cache: 'no-store' },
    );
    if (response.status === 401) {
      redirectToLoginWithNext();
      throw new ProjectEventsHttpError(401, '로그인이 만료되었습니다.');
    }
    if (response.status === 404) {
      terminalStatusRef.current = 404;
      throw new ProjectEventsHttpError(404, '워크스페이스가 종료되었거나 삭제되었습니다.');
    }
    if (!response.ok) {
      const retryAfterMs = response.status === 429
        ? parseRetryAfterHeader(response.headers.get('Retry-After'))
        : null;
      throw new ProjectEventsHttpError(
        response.status,
  ...
  }, [appendIncomingEvents, enabled, projectId, chatId, includeUnassigned]);
  ...
  const loadOlder = useCallback(async (): Promise<{ loadedCount: number; hasMoreBefore: boolean }> => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/runtime/projects/${encodeURIComponent(projectId)}/events?${params.toString()}`,
        { cache: 'no-store' },
      );
      if (response.status === 401) {
        redirectToLoginWithNext();
      ...
      if (!response.ok) {
        const retryAfterMs = response.status === 429
          ? parseRetryAfterHeader(response.headers.get('Retry-After'))
          : null;
        throw new ProjectEventsHttpError(response.status, `이전 이벤트 API 응답 오류 (${response.status})`, retryAfterMs);
      }
      ...
      if (error instanceof ProjectEventsHttpError && error.status === 429) {
  ...
  }, [projectId, chatId, includeUnassigned]);
  ...
  const resetToLatestWindow = useCallback(async () => {
    ...
    const promise = (async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new ProjectEventsHttpError(response.status, `최신 이벤트 API 응답 오류 (${response.status})`);
        }
    ...
    return promise;
  }, [chatId, includeUnassigned, projectId]);
  ...
  useEffect(() => {
    ...
    const handleRefreshError = (error: unknown, fallbackMessage: string) => {
      ...
      if (error instanceof ProjectEventsHttpError && error.status === 404) {
    ...
    const openEventSource = (streamChatId: string | null) => {
      ...
      const stream = new EventSource(
        `/api/runtime/projects/${encodeURIComponent(projectId)}/events/stream${query ? `?${query}` : ''}`,
      );
      eventSource = stream;
      let opened = false;
  ...
  }, [appendIncomingEvents, enabled, projectId, chatId, includeUnassigned, refreshEvents, waitForInitialClientSync]);
