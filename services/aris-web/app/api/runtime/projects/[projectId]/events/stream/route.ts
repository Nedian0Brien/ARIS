...
import { streamProjectEvents, getProjectRealtimeEvents, HappyHttpError } from '@/lib/happy/client';
...
export async function GET(
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      writeEvent('ready', { projectId, now: new Date().toISOString() });
      ...
      void (async () => {
        while (!aborted) {
          try {
            ...
            try {
              const rt = await getProjectRealtimeEvents({ projectId, afterCursor: realtimeCursor, chatId });
              if (aborted) {
                break;
              }
            ...
            {
              const result = await streamProjectEvents(projectId, {
