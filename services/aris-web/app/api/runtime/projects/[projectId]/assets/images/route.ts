...
function buildAttachment(input: {
  ...
  const serverPath = path.join(CHAT_IMAGE_ASSET_ROOT, input.projectSegment, `${input.assetId}-${safeName}`);
  return {
    ...
    serverPath,
    previewUrl: `/api/runtime/projects/${encodeURIComponent(input.projectSegment)}/assets/images?path=${encodeURIComponent(serverPath)}`,
  };
}
...
function isAllowedAssetPathForProject(filePath: string, projectSegment: string): boolean {
...
export async function POST(
  ...
  const attachment = buildAttachment({ projectSegment, assetId, file });
...
export async function GET(
  ...
  if (!serverPath || !isAllowedAssetPath(serverPath) || !isAllowedAssetPathForProject(serverPath, projectSegment)) {
