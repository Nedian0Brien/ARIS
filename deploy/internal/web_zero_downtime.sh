...
# Production serves aris.lawdigest.kr from the multi-service site file, which
# includes the aris-web upstream snippet. A stale default silently skips the
# nginx upstream switch (site-not-found -> deploy exits 0 but traffic stays on
# the old slot). Override with ARIS_NGINX_SITE if the host differs.
